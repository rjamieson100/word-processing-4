...
def save_words():
    """
    Saves the current word list to the file

    O(n) needs to loop through and save each word in the list (n)
    """
...
def edit_file(selection, index, word):
    """
    Handles all coded edits to the file

    @param selection: The selected edit (add, insert, remove, change)
    @param index: the index to change, occassionally unused (ex. add, remove)
    @param word: the word to introduce to the list, not used with remove.
    """
    ...
    if selection == 1:
        ...
        print("\n")
        print("(l) remove by line")
        print("(w) remove by word")
...
def here_or_there(word_list, letter, num_1, num_2):
    ...
    num_1_counter = 0
    num_2_counter = 0
    invalid_index = True #ensures the given indecies are all in range of the words.
    invalid_letters = True #ensures that only one letter is entered.

    if (num_1 >= 0) and (num_2 >= 0) and ((len(letter)) == 1):
        #check indexes are over/equal to 0 and only one letter is entered
        invalid_letters = False
        for x in (word_list):
            if (len(x) >= 5) and (num_1 < len(x)) and (num_2 < len(x)):
                #make sure the word is 5 letters or longer 
                #and the indecies aren't out of range.
                invalid_index = False
                if x[num_1] == letter:
                    num_1_counter += 1
                    #if the letter is found at the first index, increment it's counter by 1

                if x[num_2] == letter:
                    num_2_counter += 1
                    #if the letter is found at the second index, increment it's counter by 1
    if invalid_letters:
        return -3
    elif invalid_index:
        return -2
...
def menu_phase(phase):
    ...
    O(n): Single loop dependant on the ascii characters in the 
          selected button (n).
...
def function_menu():
    ...
    O(n): Single loop dependant on the amount of inputs
          by the user (n). 
    """
    selection = 1
    move = None
    ...
    if selection == 1:
        print('Run the "VowelPositionFinder.class" file. ')
        ...
        while (not word_to_solve.isalpja()):

            print("\n")
            print("input a word")
            print("\n")

            word_to_solve = input("Word: ")
        ...
        while (not letter.isalpha()) or (not num_1.isdigit()) or (not num_2.isdigit()):
           
            print("\n")
            print("input a letter and two indecies.")
            print("\n")
        ...
        common_index = here_or_there(words_list, letter, int(num_1), int(num_2))
...
def words():
    ...
    O(n): Single loop dependant on the amount of words in the file (n).
    """
    for x in range(100):
        print("\n")
    ...
    while True:
        ...
        if key == "m":
            ...
            print("\n")
            print("(x) to cancel")
            print("(a) to add a word")
            print("(i) to insert a word")
            print("(r) to remove a word")
            print("(c) to change a word")
            while True:
                edit_key = getkey()
                if edit_key == "a":
                    print("\n")
                    ...
                    word = input("Word: ")
                    line = input("Line: ")
                    while (not word.isalpha()) or (not line.isdigit()):

                        print("\n")
                        print("input a word and a line")
                        print("\n")

                        word = input("Word: ")
                        line = input("Line: ")
                    edit_file(2, (int(line) - 1), word)
                    words()
                elif edit_key == "r":
                    edit_file(3, 0, "")
                elif edit_key == "c":
...
def credits():
    ...
    O(1): Always has the same runtime as there is no input.
    """
    ...
    print("𝐂𝐥𝐢𝐞𝐧𝐭: " + "Monarch Park Collegiate Institute")
    ...
    print("𝐈𝐧𝐬𝐭𝐫𝐮𝐜𝐭𝐢𝐨𝐧𝐬: " + "Aidan McFadden")
    print("")
...
def menu():
    ...
    O(n): single loop dependant on the amount of moves the user
        inputs (n).
    """
    option = 1
    move = None
